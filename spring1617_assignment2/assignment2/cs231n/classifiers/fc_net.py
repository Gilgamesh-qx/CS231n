...
class TwoLayerNet(object):
    ...
    def loss(self, X, y=None):
        ...
        ############################################################################
        hidden,affine_cache = affine_relu_forward(X,self.params['W1'],self.params['b1'])
        ...
        dx,dw1,db1 = affine_relu_backward(dhidden, affine_cache)
...
class FullyConnectedNet(object):
    ...
    def __init__(self, hidden_dims, input_dim=3*32*32, num_classes=10,
        ...
        for i in range(len(hidden_dims)):
            ...
            self.params['W'+str(i+1)] = np.random.normal(0,weight_scale,(input_dim,layer_dim))
            self.params['b'+str(i+1)] = np.zeros(layer_dim)
                                                         
            if self.use_batchnorm:
                self.params['gamma'+str(i)] = np.ones(layer_dim)
                self.params['beta'+str(i)] = np.zeros(layer_dim)
            input_dim = layer_dim  #update the number of input_dim
                                                         
        # Add the layer which before softmax
        self.params['W'+str(self.num_layers)] = \
                           np.random.normal(0,weight_scale,(hidden_dims[-1],num_classes))
    ...
    def loss(self, X, y=None):
        ...
        for i in range(self.num_layers - 1):
            hidden,cache[i] = affine_relu_forward(hidden,
                                          self.params['W'+str(i+1)],self.params['b'+str(i+1)])
        
        out,cache[self.num_layers-1] = affine_forward(hidden,
